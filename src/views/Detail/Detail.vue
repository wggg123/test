          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="saleAttr in spuSaleAttrList" :key="saleAttr.id">
                ...
                <dd changepirce="0" @click="changeSale(attrVal,saleAttr.spuSaleAttrValueList)" :class="{active:attrVal.isChecked== 1}" v-for="attrVal in saleAttr.spuSaleAttrValueList" :key="attrVal.id">{{attrVal.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" @change="changeNum" v-model="skuNum">
...
export default {
  ...
  methods: {
    ...
    async addCart() {
      try {
        await this.$store.dispatch('addCartSuccess', { id: this.skuInfo.id, num: this.skuNum })
        this.$router.push(`/addcartsuccess/${this.skuInfo.id}/${this.skuNum}`)
        sessionStorage.setItem('SKUINFO', JSON.stringify(this.skuInfo))
        sessionStorage.setItem('SPUINFO', JSON.stringify(this.spuSaleAttrList))
      } catch (error) {
        alert('faile')
      }
    }
