<template>
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        <label>确认密码:</label>
        <input placeholder="请输入确认密码" v-model="passwordTest" name="passwordTest" v-validate="{ required: true, is: password }" :class="{ invalid: errors.has('passwordTest') }" />
        <span class="error-msg">{{ errors.first("passwordTest") }}</span>
      </div>
      <div class="controls">
    ...
    async register() {
      ...
      if (success) {
        try {
          ...
          await this.$store.dispatch('getRegister', { phone, code, password })
          alert('success')
